## About

Web projects covering interactive interfaces, dynamic functionality and responsive design

## Project Navigation

* [Color Palette Generator](./color-palette-generator/) – Automated tool for random palette generation and hex code extraction
* [Currency Converter](./currency-converter/) – Currency converter with live exchange rates and global currency support
* [Expense Tracker](./expense-tracker/) – Financial management system with real-time balance tracking and transaction history
* [Log In Form](./log-in-form/) – Secure authentication system featuring password hashing and session management
* [Loruki Website](./loruki-website/) – Responsive cloud hosting platform for deploying web applications
* [Pricing Cards](./pricing-cards/) – Responsive pricing page with animated billing toggle and plan comparison
* [Quiz](./quiz/) – Interactive quiz game with dynamic scoring and visual progress tracking
* [Weather App](./weather-app/) – Weather app displaying live conditions, dynamic icons and forecasts for any city worldwide

## Tech Stack

[![Skills](https://skillicons.dev/icons?i=html,css,js,php,mysql)](https://skillicons.dev)

## Contact

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rahmanmahmutovi%C4%87)
